...
with open("corpus.txt", "r", encoding="utf-8") as f:
    corpus_raw = f.read()
    corpus_sentences = corpus_raw.split(".")
...
def main() -> None:
    ...
    llm = pipeline('text-generation', model=LLMModel)

    with open("rag_comparison.txt", "w", encoding="utf-8") as f:
        for idx, query in enumerate(search_queries):
            ...
            paragraphs = [p for p in corpus_raw.split("\n") if len([True for i in top_matches if corpus_sentences[i[0]] in p]) > 0]
            paragraphs = "\n".join(paragraphs)
            prompt = f"Context:\n{paragraphs}\nQuery:\n{query}\nAnswer:\n"
            llmresult = llm(prompt)
            f.write(f"\nQuery [{idx}]: {query}\n")
